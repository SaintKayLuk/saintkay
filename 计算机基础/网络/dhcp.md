...
服务名
    ...
    dhcrelay        #dhcp中继服务
端口
    udp 67          #客户端的目标端口，接受客户端的dhcp请求
    udp 68          #服务端的源端口，用来回复客户端数据
配置文件
    /etc/dhcp/dhcpd.conf
```

默认配置文件为空，模板在 /usr/share/doc/dhcp*/dhcpd.conf.example
```sh
...
subnet 10.5.5.0 netmask 255.255.255.224 {
  range 10.5.5.26 10.5.5.30;                            #dhcp地址池
  option domain-name-servers ns1.internal.example.org;  #dns域
  option domain-name "internal.example.org";            #dns服务器地址
...
修改完配置文件后。启动 dhcpd 服务
```
systemctl start dhcpd
```

dchp 日志在 /var/log/message
