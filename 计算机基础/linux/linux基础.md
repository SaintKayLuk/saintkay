...
是 $PS1 的值，在/etc/bashrc 中设置，但是最好不要修改此文件，可以在自己的 .bashrc 下设置 PS1 变量
...
#### 语言支持
这就是在开机设置里面设置的语言支持，按顺序，第一位就是默认的系统语言
就是系统默认语言，比如查看一些命令的帮助的时候，会有些是会以中文显示
```sh
# 查看当前语言
locale

# 查看系统支持的语言，可以用grep过滤一下
...
Device: 802h/2050d	Inode: 1054174     Links: 1
Access: (0644/-rw-r--r--)  Uid: (    0/    root)   Gid: (    0/    root)
Context: system_u:object_r:lib_t:s0
Access: 2024-01-26 16:28:37.490000052 +0800
Modify: 2020-10-02 01:08:50.000000000 +0800
Change: 2024-01-25 16:27:25.112781093 +0800
 Birth: -



Access        最后访问时间
Modify        最后修改时间，指修改了文件内容
Change        最后一次对文件属性改变的时间，包括权限，大小，属性
Birth          文件创建时间，已被启用

一般来说修改了文件内容，则 Modify 和 Change 都会更新，修改文件属性等，Change 会更新，但是 Modify 不会更新
```
...
目录默认链接数是2，因为进入目录内能看见一个 . 是指向当前目录
至于一个目录创建出来默认 是 4096 则是linux规定，显示，当然当一个文件写了东西之后，虽然显示不到4096字节，但是用 du 统计占用空间的时候还是显示4096的倍数

##### 硬链接和软连接最大区别
文件名 --> inode --> data block

文件名存在于上级目录的data block中，权限也看上级目录
所以硬链接 只是 加了一个 文件名到 inode 的映射关系，不创建新的inode和datablock
而软连接则创建了一个新的 inode 和 data block，block中指向源文件目录

##### 硬链接

*  源文件和硬链接文件拥有相同的 inode 和 data block
*  修改任一个文件，另一个都改变
*  删除任一个文件，另一个都能使用，链接数减1，如上 第三列的2，代表2个链接，删除最后一个硬链接或者源文件，链接数为0时，同时删除block
*  普通用户不能链接目录
   *  如果 /aaa 和 /aaa/bbb 互为硬链接，则遍历的时候会引起死循环，而系统能判断软连接的符号，所以软链接不会死循环
   *  
*  不能跨分区，因为硬链接不会创建新的inode，只在原inode的inode link count 域里加1


##### 软链接

* 软链接和源文件拥有不同的 inode 和 data block
* 修改任一个文件，另一个都改变
...
**删除软链接**
* 文件软链接可以直接删除
* 目录软链接删除时目录后不能跟/，否则会删除源目录下所有文件，目录软链接还在
```sh
...
[root@localhost tmp]$ rm -rf lnaaa/
[root@localhost tmp]$ rm -rf lnbbb
[root@localhost tmp]$ tree
.
├── aaa
├── bbb
...
```sh
find    搜索路径    [选项]    搜索内容
    -maxdepth       #搜索深度，例如 -maxdepth 1，只搜索当前目录下，不搜索子目录
```
...
find . -name "abc*"   搜索当前目录下的文件名为abc开头的文件 
~~~
...
# 查找指定时间之后
find . -newermt "2023-01-01 12:00:00"
# 查找指定时间之前
find . ! -newermt "2023-01-01 12:00:00"
```
...
例：查看网关ip
netstat -rn
例：查看占用 2222 端口的应用的pid
netstat -ntupl|grep 2222 
~~~
...
tcp        0      0 0.0.0.0:111             0.0.0.0:*               LISTEN      17064/rpcbind        
tcp        0      0 0.0.0.0:20048           0.0.0.0:*               LISTEN      4507/rpc.mountd     
tcp        0      0 192.168.32.1:53         0.0.0.0:*               LISTEN      2022/named          
tcp        0      0 172.17.0.1:53           0.0.0.0:*               LISTEN      2022/named          
tcp        0      0 192.168.1.11:53         0.0.0.0:*               LISTEN      2022/named             
tcp6       0      0 :::6379                 :::*                    LISTEN      25812/docker-proxy  
tcp6       0      0 :::111                  :::*                    LISTEN      17064/rpcbind       
tcp6       0      0 :::20048                :::*                    LISTEN      4507/rpc.mountd     
tcp6       0      0 ::1:53                  :::*                    LISTEN      2022/named          
udp        0      0 192.168.1.11:53         0.0.0.0:*                           2022/named          
udp        0      0 127.0.0.1:53            0.0.0.0:*                           2022/named  

Proto               协议类型，有tcp，udp等
Recv-Q              网络接受列队，通常情况下为0
Send-Q              网络发送列队，通常情况下为0
Local Address       由两部分组成，例如 0:0:0:0:111，代表监听本机所有ip的端口22
                    如果有2个ip 11.22.33.44和44.33.22.11，那么0:0:0:0:111表示监听11.22.33.44和44.33.22.11和127.0.0.1三个ip的111端口
                    :::111是tcp6的表示方式，::代表本机所有tcp6的ip，即(0:0:0:0:0:0:0:0)

Foreign Address     与本机端口通信的外部地址。显示规则与Local Address相同
```
...
把内容写入到 a.txt 中，其中 EOF 可以替换为任意字符，但是需要保证前后一致
```sh
cat > a.txt << EOF
hahahaha
ababab
