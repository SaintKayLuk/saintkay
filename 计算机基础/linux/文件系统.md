...
* inode(i节点)：记录文件的权限，文件的所有者和所属组，文件大小等等，文件的数据真正保存的 block 编号，每个文件需要占用一个 inode
...
# 查看指定分区的uuid，例如查看 /dev/sdb1 分区的uuid
blkid /dev/sdb1
~~~
...
**不创建PV，其实也是可以VG直接从硬盘和分区直接添加，但是为了后期好维护，排查，建议创建PV**
~~~
pvcreate 设备名/分区名
...
```
例：从testvg卷组中使用所有容量，创建名为testlv的逻辑卷
```
...
lvresize [选项] 逻辑卷设备文件名
    -L  容量：单位MB,GB,TB等，使用 + 代表增加空间，- 代表减少空间，如果直接写容量，代表指定逻辑卷大小
    -l  个数：根据PE个数 调整逻辑卷大小
...
* 在线扩容
使用所有剩余vg容量，vg名为datavg，lv名为datalv
```
...
~~~
# 如果是ext类型的文件系统，使用 resize2fs
resize2fs [选项] 设备文件名 [调整的大小]
    -f  强制调整

# 如果是 xfs 类型的文件系统，使用 xfs_growfs
xfs_growfs ...
...
2. 重建完成后，虚拟磁盘容量显示增加
3. 重启服务器
4. 重新加载pv大小，因为从一整块硬盘 /dev/sdb 创建出的物理卷
   ```
   pvresize /dev/sdb
   ```
5. 在线扩容lv容量，vg名为datavg，lv名为datalv
   ```
   lvextend -l +100%FREE /dev/datavg/datalv
   ```
6. 扩展文件系统
   ```
   如果是ext4文件系统，则用 resize2fs 命令来扩展文件系统
   resize2fs /dev/datavg/datalv

   如果是xfs文件系统，则用 xfs_frowfs 命令来扩展文件系统
