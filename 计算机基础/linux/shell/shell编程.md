...
输出格式
    ...
    \v      垂直输出退格键，也就是 Tab 建
```
例：使用 printf 输出 test.txt 文件的内容,printf后面不能跟文件名，我们通过把命令结果作为 printf 得输出内容
```
...
例：获取 abc.txt 文件总行数和总列数
```sh
#查看总行数
awk 'END{print NR}' abc.txt
cat abc.txt | awk 'END{print NR}'
#查看总列数
awk '{print NF}' abc.txt
```

例：读取 abc.txt 文件第三行，如果要用变量，可以使用 双引号 "
```sh
awk 'NR==3' abc.txt
awk "NR==3" abc.txt
```
...
例：计算abc.txt 文件中第一列大于0的第二列的平均数
```sh
#abc.txt 文件内容
cat abc.txt

0 0.0 True 1577750400000
5 501.0 False 1577750400000
2 0.0 True 1577750400000
0 3.0 True 1577750400000
3 25476.6 False 1577750400000
```
...
sum=0
count=0
#awk 'END{print NR}'得总行数
for (( i=1; i<=$(awk 'END{print NR}' abc.txt); i++))
  do   
    #awk "NR==$i" 第几行
    a1=$(awk "NR==$i" abc.txt | awk '{print $1}')
    if [ $a1 -gt 0 ];then
      #第一列大于0的时候取第二列，并加到sum中，并记录count
      a2=$(awk "NR==$i" abc.txt | awk '{print $2}')
      sum=$(awk 'BEGIN{print "'$sum'" + "'$a2'"}')
      count=$(( $count+1 )) 
    fi  
  done
#有小数，用awk计算，总数除以符合的行数
...
例：打印第二行数据
```sh
[saint@localhost ~]$ sed -n '2p' test.txt
22222
```
例：打印第二行到第四行数据
```sh
[saint@localhost ~]$ sed -n '2,4p' test.txt
22222
33333
44444
```
...
55555
```
例：替换字符串，并用变量，用双引号引起来，替换test.txt文件中abc为变量AAA
```
sed -i "s/abc/$AAA/g" test.txt
```

```sh
#替换第四行，前面保留空格，使用反斜杠\来设定反斜杠后的空格也会当作替换内容
sed  "4c\    abc" test.txt
#替换第二和第五行为 xxxx
sed '2,5c xxxx' test.txt
#支持正则，替换s开头的行为 xxxx
sed '/^s/c xxxx' a.txt
#支持正则，删除包含xxx的行
sed -i '/xxx/d' a.txt
#在包含aaa或者bbb的行首添加注释#
sed -i  '/aaa/s/^/#/;/bbb/s/^/#/' a.sql
```
