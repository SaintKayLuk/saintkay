...
因为crontab最小到分，如果需要实现秒级，则多写几条，如下，每30秒执行一次
```
* * * * * 执行的任务
* * * * * sleep 30; 执行的任务
```
...
crontab有数据输出时，会输出到 /var/spool/mail/ 目录下
root用户执行的任务是/var/spool/mail/root
test用户执行的任务是/var/spool/mail/test

如果不想要数据输出，可以重定向到 /dev/null

```
* * * * * /root/test.sh>>/dev/null 2>>/dev/null
```
...
通过配置文件来设置定时任务需要指定执行命令的用户



两个文件，一个系统的定时任务，一个用户的定时任务
