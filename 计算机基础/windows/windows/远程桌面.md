...
---
3. 将远程桌面服务用户限制到单独的远程桌面服务会话(可选)
到这一步，可以用不通用户同时远程桌面登陆，但是不能同一用户同时登陆，禁用此选项可以让同一个用户同时远程桌面登陆
---
4. win10系统默认只能单用户远程桌面，需要修改termsrv.dll文件
...
windows server版本只需要1，2，3步就可以单用户同时远程了

win10 版本需要第 4 步才可以多个远程
